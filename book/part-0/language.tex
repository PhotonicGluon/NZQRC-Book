...
\begin{definition}[Word]
    Let $\Lang$ be a formal language. A \term{word}\index{word}\index{formal language!word} (or string) is either a finite concatenation of symbols from $\Alphabet$ in $\Lang$ or the empty string $\emptystring$.

    The \term{length}\index{word!length} of a word is the number of symbols making up the word, with the empty string having length 0.
\end{definition}
\begin{remark}[Making Words]
    Some authors may permit words to be formed from other means, not just concatenation. For example, using $\Alphabet = \{a, b\}$, a word could be formed from other words by the rule
    \[
        \square b,
    \]
    ...
    \]
    i.e. two (not necessarily distinct) words can be joined together with $bab$ in the middle and by placing $aaa$ at the front or end of the combined string. However we will stick with simple concatenation.
\end{remark}

\begin{example}[A Simple Alphabet]
    Let $\Alphabet = \{a, b\}$. Then $\emptystring$ (the empty word), $a$, $b$, $aa$, $ab$ $ba$, $bb$, $aaa$, $aab$, $aba$, $baa$, etc. are all valid words. Notice that we have not defined any rules on what words belong in our formal language -- these are just words formed from $\Alphabet$.
\end{example}

\subsection{Grammar and Well-Formed Words}
TODO: Add
