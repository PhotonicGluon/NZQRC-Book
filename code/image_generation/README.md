...
```bash
poetry run python main.py trim-images -o media/images/trimmed
```
...
Run:
```bash
poetry run python main.py transfer_images
```

For doing a dry run before the actual move:

```bash
poetry run python main.py transfer_images --dry-run
```
